...
public class WebtoolTestExtentReport {
	...
	public void creates_the_new_client_assessment_with_the_required_details() {
		...
		Set<String> riskCategories = Set.of("High", "Low", "Moderate", "VeryHigh", "ModerateHigh");

		if (actualRisk == expectedRisk) {
			ReportUtilities.logTest("PASS");
		} else if (actualRisk != expectedRisk) {
			ReportUtilities.logTest("FAIL");
		} else if (expectedRisk == null || actualRisk == null) {
			ReportUtilities.logTest("WARNING");
		} else {
			System.out.println("Webpage is taking too much time to response -- time out error");
			riskCategories.forEach(
					riskcategory -> System.out.println("System defined risk categories are: " + riskcategory + "\n"));
		}
	}
